...
function generateStructureGetUrl(baseUrl, accessToken) {
    ...
    return [baseUrl, ["structures", "?", "auth", "=", accessToken].join("")].join("/");
}
...
function generateSubscribeUrl(baseUrl, accessToken) {
    ...
    return [baseUrl, ["?", "auth", "=", accessToken].join("")].join("/");
}
...
function generateDeviceUpdatePutUrl(baseUrl, accessToken, deviceId, deviceType, keyToUpdate) {
    ...
    return [baseUrl, "devices", deviceType, deviceId, keyToUpdate].join("/") + "?auth=" + accessToken;
}
...
function generateDeviceUpdateRequestOptions(baseUrl, accessToken, deviceData, keyToUpdate, valueToUpdateWith) {
    ...
    var body = isString(valueToUpdateWith) ? valueToUpdateWith : valueToUpdateWith.toString();

    return {
        url: generateDeviceUpdatePutUrl(baseUrl, accessToken, deviceData.device_id, deviceData._deviceType, keyToUpdate),
        method: "PUT",
        followRedirect: true,
        removeRefererHeader: false,
        headers: {
...
function extractAndParseStreamUpdateEventBody(splitBody) {
    ...
    var dataBodyString = splitBody[1].split("data:")[1].trim();
...
module.exports = {
    generateStructureGetUrl: generateStructureGetUrl,
    generateServiceStreamRequestOptions: generateServiceStreamRequestOptions,
    generateDeviceUpdateRequestOptions: generateDeviceUpdateRequestOptions,
    parseStringIntoJsObject: parseStringIntoJsObject,
    extractStreamUpdateEventType: extractStreamUpdateEventType,
    extractAndParseStreamUpdateEventBody: extractAndParseStreamUpdateEventBody,
    splitStringByNewlines: splitStringByNewlines
};